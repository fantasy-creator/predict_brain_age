...
writer = SummaryWriter(log_dir="runs/9")
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        if mae < best_metric:
            ...
            torch.save(model.state_dict(), "best_metric_model_9.pth")
...
plt.legend()
plt.tight_layout()
plt.savefig("final_training_loss_curve_9.png")
plt.close()
...
if len(val_mae_values) > 0:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("final_validation_mae_curve_9.png")
    plt.close()
...
if len(lr_history) > 0:
    ...
    plt.tight_layout()
    # 保存为新图片
    plt.savefig("final_learning_rate_curve9.png")
    plt.close()

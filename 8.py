...
val_files_split = train_files[split_idx:int(len(train_files) * 0.8)]
print(f"共找到 {len(train_files)*0.8} 个样本")
...
writer = SummaryWriter(log_dir="runs/age_prediction_8")
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        if mae < best_metric:
            ...
            torch.save(model.state_dict(), "best_metric_model_8.pth")
...
plt.legend()
plt.tight_layout()
plt.savefig("final_training_loss_curve_8.png")
plt.close()
...
if len(val_mae_values) > 0:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("final_validation_mae_curve_8.png")
    plt.close()

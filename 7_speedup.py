...
from torch.optim.lr_scheduler import CosineAnnealingLR    #余弦退火优化器
...
over_idx=int(len(train_files)*0.8)
train_files_split = train_files[:split_idx]        # 0-0.6
val_files_split = train_files[split_idx:over_idx]  #0.6-0.8
...
writer = SummaryWriter(log_dir="runs/77")
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        if mae < best_metric:
            ...
            torch.save(model.state_dict(), "best_metric_model_7.pth")
...
plt.legend()
plt.tight_layout()
plt.savefig("final_training_loss_curve_7.png")
plt.close()
...
if len(val_mae_values) > 0:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("final_validation_mae_curve_7.png")
    plt.close()
...
if len(lr_history) > 0:
    plt.figure(figsize=(8, 6))
    plt.plot(range(1, len(lr_history) + 1), lr_history, marker='.', color='green', label="Learning Rate")
    plt.xlabel("Epoch")
    ...
    plt.tight_layout()
    # 保存为新图片
    plt.savefig("final_learning_rate_curve7.png")
    plt.close()

...
writer = SummaryWriter(log_dir="runs/age_prediction_5")
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        writer.add_scalar("Validation/RMSE", mae, epoch+1)

        if mae < best_metric:
            best_metric = mae
            best_metric_epoch = epoch + 1
            torch.save(model.state_dict(), "best_metric_model_5.pth")
...
plt.legend()
plt.tight_layout()
plt.savefig("final_training_loss_curve_5.png")
plt.close()
...
if len(val_mae_values) > 0:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("final_validation_mae_curve_5.png")
    plt.close()
